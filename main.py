...
def _categorize_query_text(q: str) -> str:
    q = q.casefold()
    GREETINGS = ["merhaba", "selam", "iyi günler", "günaydın", "iyi akşamlar", "hello", "hi", "hey", "nasılsın", "iyi misin", "teşekkürler", "sağol", "kolay gelsin", "merhabalar", "selamlar"]
    if any(w in q for w in GREETINGS) and len(q.split()) <= 3 and not any(w in q for w in _SIDE_EFFECT_WORDS + _INTERACTION_WORDS + _GENERAL_WORDS):
        return "greeting"
...
async def agent_node(state: AgentState) -> dict:
    ...
    query_type = out.get("query_type") or _categorize_query_text(question)
    
    if query_type == "greeting":
        if lang == "en":
            system_prompt = (
                "You are 'Smart Pharmacist Assistant (SPA)', a professional, helpful, and friendly AI. "
                "Briefly greet the user, introduce yourself, and ask how you can assist them with their medication or health questions today. "
                "Do NOT include any blank drug information templates, warnings, or medical subheadings. Keep it warm, polite, and brief."
            )
            evidence_label = "🌐 SOURCES:"
            user_prompt = f"GREETING: {question}"
        else:
            system_prompt = (
                "Sen 'Akıllı Eczacı Asistanı (AEA)' adında profesyonel, samimi ve yardımsever bir yapay zekasın. "
                "Kullanıcıyı sıcak ve nazik bir şekilde selamla, kendini tanıt ve bugün ilaçlar veya sağlıkla ilgili sorularında nasıl yardımcı olabileceğini sor. "
                "KESİNLİKLE boş ilaç bilgi şablonları, tıbbi alt başlıklar veya uyarılar ekleme. Yanıtın kısa, samimi ve profesyonel olsun."
            )
            evidence_label = "🌐 KAYNAKLAR:"
            user_prompt = f"SELAMLAMA: {question}"
    else:
        if lang == "en":
            system_prompt = (
...
def route_after_categorize(state: AgentState) -> Literal["Kural_Motoru_Node", "Vektor_Veritabani_RAG", "Asistan_LLM"]:
    state_dict = state.model_dump() if hasattr(state, "model_dump") else dict(state)
    q_type = state_dict.get("query_type")
    if q_type == "greeting":
        return "Asistan_LLM"
    if q_type == "interaction":
